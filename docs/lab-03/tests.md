...
| --- | --- | --- | --- | --- | --- | --- |
| UNIT-01 | Unit | BR-04, BR-05, AC-01 | Password validation and hashing | Valid boundaries pass, invalid rules fail, hashes differ from plaintext and verify correctly | `server/tests/lab-03/auth.api.test.ts` | Planned |
| UNIT-02 | Unit | BR-08, AC-04 | Session expiry and revocation | Expired or revoked tokens cannot authenticate | `server/tests/lab-03/auth.api.test.ts` | Planned |
...
| MIG-01 | Migration | BR-17, BR-18, AC-05 | DevelopmentRequester migration | IDs, Ticket ownership, Attachments, active state, and counts survive conversion | `server/tests/lab-03/authorization.api.test.ts` | Planned |
...
| API-02 | API | FR-02, BR-02, AC-02 | Invalid and inactive login | Both failures return the same safe authentication response | `server/tests/lab-03/auth.api.test.ts` | Planned |
...
| API-14 | API | FR-16, AC-13 | Staff Ticket Detail | Staff sees operational data, Attachments, comments, notes, and resolution indication | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Planned |
...
| API-16 | API | FR-18, AC-14 | IT Priority | Staff changes IT Priority without changing Requested Priority | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Planned |
...
| UI-04 | UI regression | FR-11, FR-12, FR-13, AC-08 | Requester regression | Create, My Tickets, Detail, Attachments, and no-selector flow continue | `client/tests/lab-02/CreateTicket.test.tsx`, `MyTickets.test.tsx`, `RequesterTicketDetail.test.tsx`, `AttachmentSection.test.tsx` | Planned |
...
| UI-07 | UI component | FR-16, FR-18, FR-19, AC-13, AC-14, AC-15, AC-16 | Staff Ticket Detail | Claim, assignment, priority, status, comments, notes, Attachments, and resolution indication work | `client/tests/lab-03/StaffTicketDetail.test.tsx` | Planned |
...
| STYLE-01 | UI style | FR-27, AC-22 | Zen Green controls | Tokens, labels, required markers, focus, badges, editable/read-only fields, and busy controls match `ui-spec.md` | `client/tests/lab-03/Login.test.tsx`, `StaffTicketQueue.test.tsx`, `StaffTicketDetail.test.tsx`, `UserManagement.test.tsx`, `client/tests/lab-02/VisualStyle.test.ts` | Planned |
...
| SEC-02 | Security | BR-12, BR-16, AC-07, AC-12, AC-16 | Direct authorization bypasses | Hidden controls are bypassed through direct API calls and still fail | `server/tests/lab-03/authorization.api.test.ts`, `comments-notes.api.test.ts` | Planned |
...
| REG-02 | Migration/regression | FR-11, FR-12, FR-13, AC-08 | Lab 2 Requester regression | Existing Ticket and Attachment behavior passes under authenticated identity | `server/tests/lab-02/create-ticket.api.test.ts`, `my-tickets.api.test.ts`, `ticket-detail.api.test.ts`, `attachments.api.test.ts` | Planned |
...
| E2E-03 | E2E | AC-18, AC-19, AC-20, AC-21, AC-22, AC-23 | Administrator flow | User list, search, filter, create, edit, password change, safety guards, forbidden access, and responsive states pass | `e2e/lab-03/user-administration.spec.ts` | Planned |
...
| AC-02 | API-02, UI-01, SEC-01, E2E-01 |
| AC-03 | API-03, API-05, UI-02, E2E-01 |
...
| AC-11 | API-13, UI-06 |
| AC-12 | API-06, SEC-02, UI-06, E2E-02 |
| AC-13 | API-14, API-15, UI-07, E2E-02 |
| AC-14 | API-16, UI-07, E2E-02 |
| AC-15 | UNIT-04, API-17, UI-07, E2E-02 |
| AC-16 | API-18, SEC-02, UI-07, E2E-02 |
| AC-17 | API-18, UI-07 |
