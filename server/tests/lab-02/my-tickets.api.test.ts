...
describe('Lab 2 My Tickets list API', () => {
  it('returns an owned, paginated list with stable default sorting and safe fields', async () => {
    const { login, findMany, count } = await createListHarness();
    const requester = await login();

    const response = await requester.agent.get('/api/tickets');
  ...
  it('applies trimmed search, every filter, ascending sort, and requested pagination', async () => {
    ...
    const response = await requester.agent.get(
      '/api/tickets?page=2&pageSize=20&search=%20VPN%20&categoryId=2&relatedSystemId=7&requestedPriority=URGENT&currentStatus=NEW&sortBy=ticketNumber&sortOrder=asc',
    );
  ...
  it('returns an empty page beyond the final page with accurate totals', async () => {
    const { app, login, findMany, count } = await createListHarness();
    const requester = await login();
    findMany.mockResolvedValue([]);
    count.mockResolvedValue(1);

    const response = await requester.agent.get('/api/tickets?page=3&pageSize=10');
  ...
  it.each([
    ['invalid page', '/api/tickets?page=0', 'page'],
    ['invalid page size', '/api/tickets?pageSize=15', 'pageSize'],
    ['long search', `/api/tickets?search=${'x'.repeat(121)}`, 'search'],
    ['invalid category', '/api/tickets?categoryId=0', 'categoryId'],
    ['invalid priority', '/api/tickets?requestedPriority=ASAP', 'requestedPriority'],
    ['invalid status', '/api/tickets?currentStatus=OPEN', 'currentStatus'],
    ['invalid sort field', '/api/tickets?sortBy=summary', 'sortBy'],
    ['invalid sort order', '/api/tickets?sortOrder=sideways', 'sortOrder'],
  ])('rejects %s with a safe 400 field error', async (_label, url, field) => {
    const { login, findMany, count } = await createListHarness();
    const requester = await login();

    const response = await requester.agent.get(url);
  ...
  it('rejects inactive reference filters without querying Tickets', async () => {
    const { login, categoryFindUnique, findMany } = await createListHarness();
    const requester = await login();
    categoryFindUnique.mockResolvedValueOnce({ id: 2, isActive: false });
    const categoryResponse = await requester.agent.get('/api/tickets?categoryId=2');
  ...
  it('returns a safe 500 when the database list query fails', async () => {
    const { app, login, findMany } = await createListHarness();
    const requester = await login();
